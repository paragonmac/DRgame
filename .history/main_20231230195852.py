...
class CustomTextEdit(QTextEdit):
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_Return or event.key() == Qt.Key_Enter:
            storedText = self.toPlainText()  # Store the current text
            self.clear()  # Clear the text edit
            print("Stored Text:", storedText)  # Example action (e.g., print)
            GameWindow.update_display(storedText)
        else:
...
class GameWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.Body = QTextEdit()  # Added missing definition of "Body"
    ...
    def update_display(text):
        localtext = text
        self.Body.setText(localtext)  # Fixed reference to "Body" by using "self.Body"
