...
def thread_start(objGame, objPlayer):
    thread_game_output = threading.Thread(target=objGame.check_game_output_queue)
    thread_game_output.daemon = True

    thread_gameLogicWorker = threading.Thread(target=objGame.main_game_loop, args=(objPlayer,))
...
def main():
    ...
    thread_start(thread_game_output, objPlayer)
